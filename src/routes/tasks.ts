...
tasks.get("/all", adminOnly, (c) => {
  const db = getDb();
  const rows = db.query("SELECT * FROM daily_tasks ORDER BY display_order, id").all();
  return c.json(rows);
});
...
tasks.post("/", adminOnly, async (c) => {
  ...
  const result = db.query(
    "INSERT INTO daily_tasks (title, description, display_order) VALUES (?, ?, ?)"
  ).run(title.trim(), description?.trim() || null, display_order || 0);
...
tasks.put("/:id", adminOnly, async (c) => {
  ...
  const db = getDb();
  db.query(
    "UPDATE daily_tasks SET title = ?, description = ?, display_order = ?, is_active = ? WHERE id = ?"
  ).run(title, description || null, display_order || 0, is_active ? 1 : 0, id);
